...
class ORCADLPredictor:
    ...
    # 模型内部变量名（CMIP6命名）
    MODEL_OCEAN_VARIABLES = ['so', 'thetao', 'tos', 'uo', 'vo', 'zos']
    MODEL_ATMO_VARIABLES = ['tauu', 'tauv']
    
    # 统计文件变量名（GODAS命名）
    STAT_OCEAN_VARIABLES = ['salt', 'pottmp', 'sst', 'ucur', 'vcur', 'sshg']
    STAT_ATMO_VARIABLES = ['uflx', 'vflx']
    ...
    # 统计文件变量 -> 模型变量 映射（反向）
    STAT_TO_MODEL_MAPPING = {v: k for k, v in MODEL_TO_STAT_MAPPING.items()}
    
    # 每个变量的通道数
    VARIABLE_CHANNELS = [16, 16, 1, 16, 16, 1]
    
    # 2D变量（无深度维度）
    SINGLE_LEVEL_VARS = ['tos', 'zos', 'sst', 'sshg','tauu','tauv','uflx', 'vflx']
    ...
    def __init__(
        ...
        if os.path.exists(checkpoint_path):
            # checkpoint_path 可能是文件（.bin）或模型目录
            if os.path.isfile(checkpoint_path):
                sd = torch.load(checkpoint_path, map_location='cpu')
                # 常见保存形式：直接 state_dict 或包含 'model_state_dict'
                ...
                # checkpoint_path 是目录，且内部应包含 config.json + pytorch_model.bin
                self.model = ORCADLModel.from_pretrained(checkpoint_path, ignore_mismatched_sizes=True)

            # 如果你想保证第二分支输出为0，在加载完成后再次 zero-init 第二分支：
            ...
            #_zero_init_module(self.model.enc_ocean2)
            #_zero_init_module(self.model.fusion2)
            #_zero_init_module(self.model.dec_ocean2)
            #_zero_init_module(self.model.enc_atmo2)
    ...
    def _normalize_data(
        ...
        if stat_var not in self.stat['mean']:
            raise ValueError(f"Statistics not found for variable: {stat_var}")
