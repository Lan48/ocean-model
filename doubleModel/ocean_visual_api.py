...
example_dir='/mnt/data/zhu.yishun/ORCA-DL-main/data/godas2025-12'
...
# ocean, atmo 的分别归集
ocean_keys = ['so', 'thetao', 'tos', 'uo', 'vo', 'zos']
atmo_keys  = ['tauu', 'tauv']

# 载入统计量
variables = list(variable_map.values())
stat = {
    'mean': {v: np.load(f"/mnt/data/zhu.yishun/ORCA-DL-main/stat/mean/{v}.npy") for v in variables},
    'std': {v: np.load(f"/mnt/data/zhu.yishun/ORCA-DL-main/stat/std/{v}.npy") for v in variables}
}
...
def downsample_matrix(mat: np.ndarray, out_shape=(64, 64)) -> list:
    """简单的最近邻下采样，返回 list 列表格式"""
    try:
        h, w = mat.shape
    except Exception:
        return []
    ...
    if h == 0 or w == 0:
        return []
    rows = (np.linspace(0, h - 1, nh)).astype(int)
    cols = (np.linspace(0, w - 1, nw)).astype(int)
    out = mat[rows[:, None], cols[None, :]]
    return sanitize_for_json(out)


def compute_spatial_stats(mat: np.ndarray, lats: np.ndarray, lons: np.ndarray, top_pct: float = 5.0) -> Dict:
    """计算一组用于 VLM 的空间统计量，返回可 JSON 序列化的字典"""
    out = {}
    try:
        ...
        total_cells = a.size
        valid_count = valid_vals.size
        # 已不返回全量有效格点比例，以减小返回字段列表
        if valid_count == 0:
            # empty
            out.update({'mean': None, 'std': None, 'min': None, 'max': None})
            return out
        out['mean'] = round(float(np.nanmean(a)), 3)
        out['std'] = round(float(np.nanstd(a)), 3)
        ...
        # min/max locations
        try:
            idx_min = np.nanargmin(a)
            idx_max = np.nanargmax(a)
            rmin, cmin = np.unravel_index(idx_min, a.shape)
            rmax, cmax = np.unravel_index(idx_max, a.shape)
            out['min_loc'] = [float(lons[cmin]), float(lats[rmin])]
            out['max_loc'] = [float(lons[cmax]), float(lats[rmax])]
        except Exception:
            out['min_loc'] = None
            out['max_loc'] = None
        ...
        # percentiles
        try:
            p10, p25, p50, p75, p90 = np.nanpercentile(a, [10, 25, 50, 75, 90])
            out['p10'] = round(float(p10), 3)
            out['p25'] = round(float(p25), 3)
            out['p50'] = round(float(p50), 3)
            out['p75'] = round(float(p75), 3)
            out['p90'] = round(float(p90), 3)
        except Exception:
            pass

        # centroid of top X% (by value)
        try:
            pct = 100.0 - float(top_pct)
            thr = float(np.nanpercentile(a, pct))
            mask_top = np.logical_and(valid_mask, a >= thr)
            if np.any(mask_top):
                ys, xs = np.where(mask_top)
                vals = a[ys, xs]
                # weight by value
                W = np.nansum(vals)
                if W > 0:
                    lon_cent = float(np.nansum(lons[xs] * vals) / W)
                    lat_cent = float(np.nansum(lats[ys] * vals) / W)
                    out['centroid_top{}_pct'.format(int(top_pct))] = [round(lon_cent, 3), round(lat_cent, 3)]
                else:
                    out['centroid_top{}_pct'.format(int(top_pct))] = None
            else:
                out['centroid_top{}_pct'.format(int(top_pct))] = None
        except Exception:
            out['centroid_top{}_pct'.format(int(top_pct))] = None

        # exceedance simple metrics
        try:
            out['frac_gt_0'] = round(float(np.sum(a > 0) / float(valid_count)), 3)
        except Exception:
            out['frac_gt_0'] = None
...
'''
def fig_to_base64(fig, quality=60, dpi=80) -> str:
    buf = io.BytesIO()
    fig.savefig(buf, format='png', dpi=dpi, bbox_inches='tight')
    plt.close(fig)
    buf.seek(0)
    from PIL import Image
    img = Image.open(buf).convert("RGB")  # JPEG 不支持 alpha
    out = io.BytesIO()
...
def create_time_series_plot(
    ...
    if allow_downsample:
        ...
        # 不降采样：为每个点设置刻度，标签由调用者控制（可为空字符串以跳过显示）
...
def create_enso_plot(
    ...
    # 处理 x 轴标签：去掉 'Month' 前缀，保留调用方提供的标签位置
    cleaned_labels = []
    for lab in x_labels:
    ...
    # 不进行降采样：将每个位置都设置为刻度，调用方可以传入空字符串以隐藏非必要标签
    n = len(cleaned_labels)
    ax.set_xticks(list(range(n)))
    ax.set_xticklabels([cleaned_labels[i] for i in range(n)])
    ...
    for p in nc_paths:
        if os.path.exists(p):
            try:
                ...
                # 选择数据变量名：优先映射名，其次短名，最后取第一个变量
                varname_in_ds = fname if fname in ds.variables else (key if key in ds.variables else None)
                if varname_in_ds is None:
                    # 作为回退，取 datasets 中第一个变量名
                    varname_in_ds = list(ds.variables.keys())[0]
...
class SeasonalMeanResponse(BaseModel):
    attribute_matrix: Dict = Field(..., description="返回结构：{'stats':统计字典} (已移除完整栅格与下采样预览，返回仅包含摘要统计)" )
...
class ENSOForecastResponse(BaseModel):
    attribute_matrix: Dict = Field(..., description="返回结构：{'series':列表,'stats':统计字典,'rolling_mean_3mo':列表}")
...
class MarineHeatwaveResponse(BaseModel):
    attribute_matrix: Dict = Field(..., description="返回结构：{'stats':统计字典,'threshold_summary': dict,'pct_exceed_1':float,'pct_exceed_3':float} (已移除完整栅格与下采样预览)")
...
class DecadalPredictionRequest(BaseModel):
    ...
    lon_max: float = Field(..., ge=0.5, le=359.5, description="经度最大值（0.5~359.5，需>lon_min）")
    var_name: VarNameEnum = Field(..., description="变量名（tos/zos/thetao等）")
    # 不再使用 predict_steps：以 start_offset/end_offset 指定要截取的预测区间（相对于 start_month 的偏移，单位：月）
    start_offset: int = Field(0, ge=0, description="起始偏移（月），相对于 start_month 的月数，>=0")
    end_offset: int = Field(..., ge=0, description="结束偏移（月），相对于 start_month 的月数，需>= start_offset；决定模型需要预测到的最远步")
    ...
    @validator('end_offset')
    def end_offset_ge_start(cls, v, values):
        so = values.get('start_offset', 0)
        if v is None:
            raise ValueError('end_offset must be provided')
        if v < so:
            raise ValueError('end_offset must be >= start_offset')
        return v
...
class DecadalPredictionResponse(BaseModel):
    attribute_matrix:  Dict = Field(..., description="返回结构：{'series':列表,'stats':统计字典}")
    attribute_description: str = Field(..., description="年代际预测属性描述信息")
...
async def startup():
    ...
    try:
        ...
        predictor = ORCADLPredictor(
            ...
            checkpoint_path='/mnt/data/zhu.yishun/ORCA-DL-main/double_output2e-7/pytorch_model.bin',
...
async def seasonal_mean_visual(request: SeasonalMeanRequest):
    ...
    try:
        ...
        for key in ocean_keys:
            ...
            ocean_data[varname] = arr.astype(np.float32)

        for key in atmo_keys:
            varname, arr = load_example_for_key(key)
            if arr is None:
                continue
            atmo_data[varname] = arr.astype(np.float32)
        ...
        # 计算统计量并构建返回结构（仅返回统计摘要以减小负载）
        stats = compute_spatial_stats(sliced_data, actual_lats, actual_lons)
        attribute_payload = {
            'stats': stats
        }

        return SeasonalMeanResponse(
            attribute_matrix=attribute_payload,
...
async def enso_forecast_visual(request: ENSOForecastRequest):
    ...
    try:
        ...
        for key in ocean_keys:
            _, arr = load_example_for_key(key)
            if arr is None:
                continue
            ocean_data[key] = arr.astype(np.float32)

        for key in atmo_keys:
            _, arr = load_example_for_key(key)
            if arr is None:
                continue
            atmo_data[key] = arr.astype(np.float32)
        ...
        x_labels_for_plot = []
        for idx, step in enumerate(steps_used):
            # 计算该 step 对应的日历月份与年份（以 base_year 为起始年）
            month_idx = ((start_month_idx + step) % 12) + 1
            year = base_year + ((start_month_idx + step) // 12)
            # 首个标签必须展示；每年一月也必须展示，格式为 年-月（YYYY-MM）；其余位置用空字符串占位
            if idx == 0 or month_idx == 1:
                x_labels_for_plot.append(f"{year}-{month_idx:02d}")
            else:
        ...
        attr_desc = (
            ...
            f"起始月份（输入）={request.start_month}，请求预测步长={request.predict_steps}月，使用偏移窗口 start_offset={start_offset}, end_offset={end_offset}；"
        ...
        # 计算序列统计量并返回结构化结果
        try:
            ser = np.array(anomalies, dtype=float)
            ser_stats = {
                'mean': round(float(np.nanmean(ser)), 3),
                'std': round(float(np.nanstd(ser)), 3),
                'min': round(float(np.nanmin(ser)), 3),
                'max': round(float(np.nanmax(ser)), 3),
                'p50': round(float(np.nanpercentile(ser, 50)), 3),
                'p90': round(float(np.nanpercentile(ser, 90)), 3),
                'length': int(len(ser))
            }
            # trend (per step)
            if len(ser) > 1:
                x = np.arange(len(ser))
                z = np.polyfit(x, ser, 1)
                ser_stats['trend_slope_per_step'] = round(float(z[0]), 6)
            else:
                ser_stats['trend_slope_per_step'] = None
        except Exception:
            ser_stats = {}
...
async def marine_heatwave_visual(request: MarineHeatwaveRequest):
    ...
    try:
        ...
        # 统计热浪矩阵的空间统计量
        stats = compute_spatial_stats(sliced_heatwave_months, actual_lats, actual_lons)
        # 计算超过不同月数阈值的格点比例
        try:
            arr = np.array(sliced_heatwave_months)
            valid = np.isfinite(arr)
            valid_count = int(np.sum(valid))
            pct_exceed_1 = round(float(np.sum(np.logical_and(valid, arr > 0)) / valid_count) if valid_count>0 else 0.0, 3)
            pct_exceed_3 = round(float(np.sum(np.logical_and(valid, arr >= 3)) / valid_count) if valid_count>0 else 0.0, 3)
        except Exception:
            pct_exceed_1 = None
            pct_exceed_3 = None
        ...
        try:
            if 'threshold_values' in locals() and threshold_values is not None:
                tv = np.array(threshold_values)
                if tv.ndim >= 2:
                    threshold_summary = {'type': 'grid', 'p90_mean': float(np.nanmean(tv)), 'p90_std': float(np.nanstd(tv))}
                else:
                    threshold_summary = {'type': 'monthly_scalar', 'p90_mean': float(np.nanmean(tv))}
        except Exception:
            threshold_summary = None
        ...
        attribute_payload = {
            ...
            'pct_exceed_1': pct_exceed_1,
            'pct_exceed_3': pct_exceed_3
        }
...
async def decadal_prediction_visual(request: DecadalPredictionRequest):
    ...
    try:
        var_name = request.var_name.value
        # 使用 start_offset/end_offset 指定要截取的预测区间（相对于 start_month 的偏移，单位：月）
        start_offset = int(getattr(request, 'start_offset', 0))
        end_offset = int(getattr(request, 'end_offset'))
        # 为使模型能产生所需区间，要求模型至少预测到 end_offset（相对于 start_month），
        # 因此向模型请求的总月数为 end_offset+1（包含第0步）
        total_months = end_offset + 1
        gran = getattr(request, 'output_granularity', 'year')
        logger.info(f"[年代际预测] 变量: {var_name}, start_offset={start_offset}, end_offset={end_offset}, 要求模型预测总月数={total_months}")
        ...
        # 从模型全部预测结果中截取用户指定的区间 [start_offset, end_offset]（包含两端）
        sliced_months = all_monthly_values[start_offset:end_offset+1]
        if len(sliced_months) == 0:
            raise HTTPException(status_code=400, detail=f"请求的 start_offset/end_offset 导致无可用预测数据: start_offset={start_offset}, end_offset={end_offset}")

        # 根据请求的输出粒度决定返回月序列还是年聚合
        if gran == 'month':
            # 按月返回：生成时间序列图（每步为月），标签从 start_month + start_offset 开始
            # 标签规则：第一个点必须有标签；每年一月必须有标签；标签格式为 YYYY-MM；其他月份使用空字符串占位
            try:
                base_year = int(os.environ.get('ORCA_DL_ENSO_START_YEAR', 2026))
            except Exception:
                base_year = 2026
            ...
            month_labels = []
            for i in range(len(sliced_months)):
                abs_month_idx = start_month_idx + start_offset + i
                month_num = (abs_month_idx % 12) + 1
                year = base_year + (abs_month_idx // 12)
                if i == 0 or month_num == 1:
                    month_labels.append(f"{year}-{month_num:02d}")
                else:
                    month_labels.append("")

            visual_base64 = create_time_series_plot(
                values=sliced_months,
            ...
            attr_desc = (
                ...
                f"总月数（截取区间）：{len(sliced_months)}（start_offset={start_offset}, end_offset={end_offset}）；"
                f"输出经纬度范围：纬度 {lat_min_out:.2f}°–{lat_max_out:.2f}°，经度 {lon_min_out:.2f}°–{lon_max_out:.2f}°；"
                f"深度信息：{depth_info}；"
                f"维度说明：[总月数({len(sliced_months)})]"
            )
            ...
            return DecadalPredictionResponse(
                attribute_matrix={
                    'series': sanitize_for_json(np.array(sliced_months)),
                    'stats': {
                        'count': len(sliced_months),
                        'mean': float(np.nanmean(sliced_months)),
                        'std': float(np.nanstd(sliced_months)),
                        'min': float(np.nanmin(sliced_months)),
                        'max': float(np.nanmax(sliced_months)),
                        'p50': float(np.nanpercentile(sliced_months, 50)),
                        'p90': float(np.nanpercentile(sliced_months, 90)),
            ...
            # 年均值分组（基于截取后的区间）
            annual_means = []
            for i in range(0, len(sliced_months), 12):
                group = sliced_months[i:i+12]
            ...
            try:
                series_stats = {
                    'count': len(annual_means),
                    'mean': float(np.nanmean(annual_means)),
                    'std': float(np.nanstd(annual_means)),
                    'min': float(np.nanmin(annual_means)),
                    'max': float(np.nanmax(annual_means)),
                    'p50': float(np.nanpercentile(annual_means, 50)),
                    'p90': float(np.nanpercentile(annual_means, 90)),
            ...
            return DecadalPredictionResponse(
                attribute_matrix={
                    'series': sanitize_for_json(np.array(annual_means)),
                    'stats': series_stats
                },
                attribute_description=attr_desc,
                visual_base64=visual_base64
