...
class ORCADLModel(ORCADLPreTrainedModel):
    ...
    def __init__(self, config: ORCADLConfig) -> None:
        ...
        self.enc_ocean2 = OceanEncoders(config)
        self.fusion2 = FusionModule(config)
        self.dec_ocean2 = OceanDecoders(config)
        self.enc_atmo2 = AtmoEncoder(config)

        # Zero-initialize the second branch modules so their initial outputs are forced to 0
        ...
        print("using double mode")
        self.post_init()
        _zero_init_module(self.enc_ocean2)
        _zero_init_module(self.fusion2)
        _zero_init_module(self.dec_ocean2)
        _zero_init_module(self.enc_atmo2)
    ...
    def forward_single_step(
        ...
        x2, enc_x2 = self.enc_ocean2(ocean_vars, lead_time, self.all_land_mask_pad, self.all_land_mask_pad_shifted, mask)
        x2 = self.fusion2(x2, atmo_vars, lead_time, atmo_lead_time, self.land_mask_pad_mix, self.land_mask_pad_shifted_mix)
        logits2 = self.dec_ocean2(x2, lead_time, enc_x2, self.all_land_mask_pad, self.all_land_mask_pad_shifted)
    ...
    def forward(
        ...
        atmo_vars1 = self.enc_atmo(atmo_vars, None, self.all_land_mask_pad, self.all_land_mask_pad_shifted, mask)
        atmo_vars2 = self.enc_atmo2(atmo_vars, None, self.all_land_mask_pad, self.all_land_mask_pad_shifted, mask)
        atmo_vars = atmo_vars1 + atmo_vars2
