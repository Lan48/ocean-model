...
class MlpMoe(nn.Module):
    ...
    def forward(self, x, leat_t):
        ...
        for i in range(self.max_t):
            ...
            if len(selected_x) == 0:
                continue
            mlp_out = mlp(selected_x)
            # Ensure dtype consistency between destination and source when using AMP/fp16
            if mlp_out.dtype != out.dtype:
                out = out.to(mlp_out.dtype)
            out[idx] = mlp_out
        return out
...
class SwinLayer(nn.Module):
    ...
    def forward(self, x, mask_matrix, mask_matrix_shifted, lead_t=None):
        ...
        if self.gradient_checkpointing:
            x = checkpoint.checkpoint(self.forward_part1, x, mask_matrix, mask_matrix_shifted, use_reentrant=False)
        else:
            x = self.forward_part1(x, mask_matrix, mask_matrix_shifted)
        x = shortcut + self.drop_path(x)

        if self.gradient_checkpointing:
            x = x + checkpoint.checkpoint(self.forward_part2, x, lead_t, use_reentrant=False)
        else:
