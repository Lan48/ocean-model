...
class ORCADLConfig(PretrainedConfig):
    def __init__(
        ...
        **kwargs,
        ...
        self.var_list = list(var_list) if var_list is not None else list(DEFAULT_MODEL_OCEAN_VARS)
        self.var_index = list(var_index) if var_index is not None else build_var_index(self.var_list)
        if out_chans is None:
            _, out_chans = build_ocean_channel_lists(self.var_list)
        if in_chans is None:
            in_chans, _ = build_ocean_channel_lists(self.var_list)
        self.in_chans = list(in_chans)
        ...
        self.atmo_var_list = list(atmo_var_list) if atmo_var_list is not None else list(DEFAULT_MODEL_ATMO_VARS)
        self.atmo_dims = atmo_dims if atmo_dims is not None else len(self.atmo_var_list)
        ...
        self.in_steps = infer_input_steps(self.in_chans, self.out_chans)
...
def _resolve_pretrained_assets(model_path):
    if os.path.isdir(model_path):
        ckpt_candidates = [
            os.path.join(model_path, "pytorch_model.bin"),
            os.path.join(model_path, "seed_1.bin"),
        ]
        config_candidates = [
        ...
        ckpt_path = model_path if os.path.exists(model_path) else None
        parent_dir = os.path.dirname(model_path)
        config_candidates = [
            os.path.join(parent_dir, "config.json"),
            os.path.join(parent_dir, "model_config.json"),
        ]
        config_path = next((path for path in config_candidates if os.path.exists(path)), None)

    if ckpt_path is None:
        raise FileNotFoundError(f"Cannot find checkpoint under {model_path}")

    source_config = ORCADLConfig.from_json_file(config_path) if config_path else None
    return ckpt_path, source_config


def _remap_var_module_key(key, prefix, source_var_list, target_var_to_idx):
    if not key.startswith(prefix):
        return key

    remainder = key[len(prefix):]
    index_str, _, suffix = remainder.partition(".")
    if not index_str.isdigit():
        return key

    source_idx = int(index_str)
    if source_idx >= len(source_var_list):
        return None

    var_name = source_var_list[source_idx]
    if var_name not in target_var_to_idx:
        return None

    return f"{prefix}{target_var_to_idx[var_name]}.{suffix}"
...
class ORCADLModel(ORCADLPreTrainedModel):
    ...
    def load_expanded_state_dict(self, model_path, source_config=None):
        checkpoint_path, inferred_source_config = _resolve_pretrained_assets(model_path)
        source_config = source_config or inferred_source_config
        ...
        source_var_list = list(getattr(source_config, "var_list", [])) if source_config is not None else list(self.config.var_list)
        target_var_to_idx = {var_name: idx for idx, var_name in enumerate(self.config.var_list)}
        adapted_state_dict = {}

        for key, value in cleaned_state_dict.items():
            remapped_key = _remap_var_module_key(
                key, "enc_ocean.encoder_list.", source_var_list, target_var_to_idx
            )
            if remapped_key is None:
                continue
            remapped_key = _remap_var_module_key(
                remapped_key, "dec_ocean.decoder_list.", source_var_list, target_var_to_idx
            )
            if remapped_key is None:
                continue

            if remapped_key in target_state and target_state[remapped_key].shape == value.shape:
                adapted_state_dict[remapped_key] = value

        source_last_dim = None
        target_last_dim = int(self.config.embed_dim * 2 ** (len(self.config.enc_depths) - 1))
        if source_config is not None:
            source_last_dim = int(source_config.embed_dim * 2 ** (len(source_config.enc_depths) - 1))

        enc_proj_key = "enc_ocean.proj.weight"
        if (
            source_last_dim == target_last_dim
            and source_config is not None
            and enc_proj_key in cleaned_state_dict
            and enc_proj_key in target_state
            and cleaned_state_dict[enc_proj_key].shape[0] == target_state[enc_proj_key].shape[0]
        ):
            expanded = target_state[enc_proj_key].clone()
            source_weight = cleaned_state_dict[enc_proj_key]
            for source_idx, var_name in enumerate(source_var_list):
                target_idx = target_var_to_idx.get(var_name)
                if target_idx is None:
                    continue
                src_slice = slice(source_idx * source_last_dim, (source_idx + 1) * source_last_dim)
                tgt_slice = slice(target_idx * target_last_dim, (target_idx + 1) * target_last_dim)
                expanded[:, tgt_slice] = source_weight[:, src_slice]
            adapted_state_dict[enc_proj_key] = expanded

        dec_proj_weight_key = "dec_ocean.proj.weight"
        dec_proj_bias_key = "dec_ocean.proj.bias"
        if source_last_dim == target_last_dim and source_config is not None:
            if (
                dec_proj_weight_key in cleaned_state_dict
                and dec_proj_weight_key in target_state
                and cleaned_state_dict[dec_proj_weight_key].shape[1] == target_state[dec_proj_weight_key].shape[1]
            ):
                expanded = target_state[dec_proj_weight_key].clone()
                source_weight = cleaned_state_dict[dec_proj_weight_key]
                for source_idx, var_name in enumerate(source_var_list):
                    target_idx = target_var_to_idx.get(var_name)
                    if target_idx is None:
                        continue
                    ...
                    tgt_slice = slice(target_idx * target_last_dim, (target_idx + 1) * target_last_dim)
                    expanded[tgt_slice] = source_weight[src_slice]
                adapted_state_dict[dec_proj_weight_key] = expanded

            if dec_proj_bias_key in cleaned_state_dict and dec_proj_bias_key in target_state:
                expanded = target_state[dec_proj_bias_key].clone()
                source_bias = cleaned_state_dict[dec_proj_bias_key]
                for source_idx, var_name in enumerate(source_var_list):
                    target_idx = target_var_to_idx.get(var_name)
                    if target_idx is None:
                        continue
                    src_slice = slice(source_idx * source_last_dim, (source_idx + 1) * source_last_dim)
                    tgt_slice = slice(target_idx * target_last_dim, (target_idx + 1) * target_last_dim)
                    expanded[tgt_slice] = source_bias[src_slice]
                adapted_state_dict[dec_proj_bias_key] = expanded

        incompatible = self.load_state_dict(adapted_state_dict, strict=False)
        return incompatible
