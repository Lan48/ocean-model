...
class MlpMoe(nn.Module):
    ...
    def forward(self, x, leat_t):
        ...
        for i in range(self.max_t):
            ...
            if len(selected_x) == 0:
                continue
            selected_out = mlp(selected_x)
            if out is None:
                out = selected_out.new_zeros((B, H, W, self.out_dims))
            out[idx] = selected_out
        if out is None:
            out = x.new_zeros((B, H, W, self.out_dims))
        return out
...
class SwinLayer(nn.Module):
    ...
    def _run_with_checkpoint(self, fn, *args):
        if not self.gradient_checkpointing or not self.training or not torch.is_grad_enabled():
            return fn(*args)
        # DDP does not support reentrant checkpoint segments that reuse the same
        # parameters within one iteration. Use the non-reentrant variant instead.
    ...
    def forward(self, x, mask_matrix, mask_matrix_shifted, lead_t=None):
        ...
        shortcut = x
        x = self._run_with_checkpoint(self.forward_part1, x, mask_matrix, mask_matrix_shifted)
        x = shortcut + self.drop_path(x)

        x = x + self._run_with_checkpoint(self.forward_part2, x, lead_t)
